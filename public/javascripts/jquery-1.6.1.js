 ...
 * 
 * Copyright 2011, John Resig Dual licensed under the MIT or GPL Version 2
 * licenses. http://jquery.org/license
 * 
 * Includes Sizzle.js http://sizzlejs.com/ Copyright 2011, The Dojo
 * Foundation Released under the MIT, BSD, and GPL Licenses.
 * 
 * Date: Thu May 12 15:04:36 2011 -0400
 */
if (typeof(window.bq_jQuery) == 'undefined') {
	(function(window, undefined) {
		...
		var jQuery = (function() {
							...
							if (match[1]) {
								...
								if (ret) {
									if (jQuery.isPlainObject(context)) {
										selector = [ document
												.createElement(ret[1]) ];
										jQuery.fn.attr.call(selector, context,
												true);
			...
			jQuery
					.extend({
						...
						ready : function(wait) {
							// Either a released hold or an DOMready/load event
							// and
							// not yet ready
							if ((wait === true && !--jQuery.readyWait)
								...
								// If a normal DOM Ready event fired, decrement,
								// and
								// wait if need be
						...
						bindReady : function() {
							...
							// Mozilla, Opera and webkit nightlies currently
							// support
							// this event
						...
						isNaN : function(obj) {
							return obj == null || !rdigit.test(obj)
									|| isNaN(obj);
						},
						...
						isPlainObject : function(obj) {
							// Must be an Object.
							// Because of IE, we also have to check the presence
							// of
							...
							// Own properties are enumerated firstly, so to
							// speed
							// up,
						...
						makeArray : function(array, results) {
							...
							if (array != null) {
								...
								// Tweaked logic slightly to handle Blackberry
								// 4.7
								// RegExp issues #6930
						...
						map : function(elems, callback, arg) {
							...
							// Go through the array, translating each of the
							// items
							// to their
							if (isArray) {
								for (; i < length; i++) {
						...
						access : function(elems, key, value, exec, fn, pass) {
							...
							if (value !== undefined) {
								// Optionally, function values get executed if
								// exec
								// is true
								exec = !pass && exec
		...
		var // Promise methods
		promiseMethods = "done fail isResolved isRejected promise then always pipe"
				.split(" "),
		...
		jQuery
				.extend({
					...
					Deferred : function(func) {
						var deferred = jQuery._Deferred(), failDeferred = jQuery
								._Deferred(), promise;
						...
						jQuery
								.extend(
										deferred,
										{
											...
											always : function() {
												return deferred.done.apply(
														deferred, arguments).fail
					...
					when : function(firstParam) {
						var args = arguments, i = 0, length = args.length, count = length, deferred = length <= 1
								&& firstParam
						...
						function resolveFunc(i) {
							return function(value) {
								...
								if (!(--count)) {
									// Strange bug in FF4:
									// Values changed onto the arguments object
									// sometimes end up as undefined values
									// outside the $.when method. Cloning the
									// object
									// into a fresh array solves the issue
									deferred.resolveWith(deferred,
											sliceDeferred.call(args, 0));
								}
							};
						}
						if (length > 1) {
		...
		jQuery.support = (function() {
			...
			if (document.defaultView && document.defaultView.getComputedStyle) {
				...
				support.reliableMarginRight = (parseInt((document.defaultView
		...
		jQuery
				.extend({
					...
					// The following elements throw uncatchable exceptions if
					// you
					...
					data : function(elem, name, data, pvt /* Internal Use Only */) {
						...
						if (!cache[id]) {
							...
							// TODO: This is a hack for 1.5 ONLY. Avoids
							// exposing
							// jQuery
							// metadata on plain JS objects when the object is
							// serialized using
							// JSON.stringify
							if (!isNode) {
						...
						if (typeof name === "object"
							...
							if (pvt) {
								cache[id][internalKey] = jQuery.extend(
						...
						// Internal jQuery data is stored in a separate object
						// inside the object's data
						...
						// TODO: This is a hack for 1.5 ONLY. It will be removed
						// in
						// 1.6. Users should
						// not attempt to inspect the internal events object
						// using
						// jQuery.data, as this
						// internal data object is undocumented and subject to
						// change.
						if (name === "events" && !thisCache[name]) {
							return thisCache[internalKey]
									&& thisCache[internalKey].events;
						}

						return getByName ? thisCache[jQuery.camelCase(name)]
								: thisCache;
					},
					...
					removeData : function(elem, name, pvt /* Internal Use Only */) {
						...
						if (name) {
							var thisCache = pvt ? cache[id][internalKey]
									: cache[id];
						...
						// Browsers that fail expando deletion also refuse to
						// delete
						// expandos on
						// the window, but it will allow it on all other JS
						// objects;
						// other browsers
						// don't care
						...
						// We destroyed the entire user cache at once because
						// it's
						// faster than
						// iterating through each key, but we need to continue
						// to
						// persist internal
						// data if it existed
						if (internalCache) {
							...
							// Otherwise, we need to eliminate the expando on
							// the
							// node to avoid
							// false lookups in the cache for entries that no
							// longer
							// exist
						} else if (isNode) {
					...
					// A method for determining if a DOM node can handle the
					// data
					// expando
					acceptData : function(elem) {
						if (elem.nodeName) {
							var match = jQuery.noData[elem.nodeName
									.toLowerCase()];
			...
			data : function(key, value) {
				...
				if (value === undefined) {
					...
					return this.each(function() {
						...
						$this.triggerHandler("changeData" + parts[1] + "!",
								args);
					});
				}
			},
		...
		function dataAttr(elem, key, data) {
			...
			if (data === undefined && elem.nodeType === 1) {
				var name = "data-"
						+ key.replace(rmultiDash, "$1-$2").toLowerCase();
		...
		function handleQueueMarkDefer(elem, type, src) {
			var deferDataKey = type + "defer", queueDataKey = type + "queue", markDataKey = type
					+ "mark", defer = jQuery.data(elem, deferDataKey,
					undefined, true);
			if (defer
					&& (src === "queue" || !jQuery.data(elem, queueDataKey,
							undefined, true))
					&& (src === "mark" || !jQuery.data(elem, markDataKey,
							undefined, true))) {
				// Give room for hard-coded callbacks to fire first
				// and eventually mark/queue something else on the element
				setTimeout(
						function() {
							if (!jQuery.data(elem, queueDataKey, undefined,
									true)
									&& !jQuery.data(elem, markDataKey,
											undefined, true)) {
		...
		jQuery
				.extend({
					...
					_mark : function(elem, type) {
						if (elem) {
							type = (type || "fx") + "mark";
							jQuery.data(elem, type, (jQuery.data(elem, type,
					...
					_unmark : function(force, elem, type) {
						...
						if (elem) {
							type = type || "fx";
							var key = type + "mark", count = force ? 0
									: ((jQuery.data(elem, key, undefined, true) || 1) - 1);
							if (count) {
					...
					queue : function(elem, type, data) {
						if (elem) {
							...
							// Speed up dequeue by getting out quickly if this
							// is just a
							// lookup
							if (data) {
								if (!q || jQuery.isArray(data)) {
									q = jQuery.data(elem, type, jQuery
											.makeArray(data), true);
								} else {
									q.push(data);
								}
					...
					dequeue : function(elem, type) {
						...
						var queue = jQuery.queue(elem, type), fn = queue
								.shift(), defer;

						// If the fx queue is dequeued, always remove the
						// progress sentinel
						if (fn === "inprogress") {
							fn = queue.shift();
						}
						...
						if (fn) {
							// Add a progress sentinel to prevent the fx queue
							// from being
		...
		jQuery.fn
				.extend({
					...
					val : function(value) {
						...
						if (!arguments.length) {
							if (elem) {
								hooks = jQuery.valHooks[elem.nodeName
										.toLowerCase()]
		...
		jQuery
				.extend({
					...
					attr : function(elem, name, value, pass) {
						...
						// don't get/set attributes on text, comment and
						// attribute
						// nodes
						...
						if (!hooks) {
							...
							if (rboolean.test(name)
								...
								// Use formHook for forms and if the name
								// contains
								// certain characters
							} else if (formHook
									&& (jQuery.nodeName(elem, "form") || rinvalidChar
											.test(name))) {
								hooks = formHook;
							}
						}
					...
					removeAttr : function(elem, name) {
						...
						if (elem.nodeType === 1) {
							...
							if (jQuery.support.getSetAttribute) {
								// Use removeAttribute in browsers that support
								// it
								elem.removeAttribute(name);
							} else {
								jQuery.attr(elem, name, "");
								elem.removeAttributeNode(elem
						...
						type : {
							set : function(elem, value) {
								...
								if (rtype.test(elem.nodeName)
									...
									// Setting the type on a radio button after
									// the
									// value resets the value in IE6-9
									// Reset value to it's default in case type
									// is
									// set after value
					...
					prop : function(elem, name, value) {
						...
						// don't get/set properties on text, comment and
						// attribute
						// nodes
						if (!elem || nType === 3 || nType === 8 || nType === 2) {
							return undefined;
		...
		boolHook = {
			get : function(elem, name) {
				...
				return elem[jQuery.propFix[name] || name] ? name.toLowerCase()
						: undefined;
			},
			set : function(elem, value, name) {
				...
				if (value === false) {
					...
					if (propName in elem) {
						// Only set the IDL specifically if it already exists on
						// the
						// element
						elem[propName] = value;
					}
		...
		if (!jQuery.support.getSetAttribute) {
			...
			formHook = jQuery.attrHooks.name = jQuery.valHooks.button = {
				...
				set : function(elem, value, name) {
					...
					// Only use nodeValue if the attribute node exists on the
					// form
			...
			// Set width and height to auto instead of 0 on empty string( Bug
			// #8150
			// )
		...
		if (!jQuery.support.style) {
			jQuery.attrHooks.style = {
				get : function(elem) {
					...
					// Normalize to lowercase since IE uppercases css property
					// names
		...
		var hasOwn = Object.prototype.hasOwnProperty, rnamespaces = /\.(.*)$/, rformElems = /^(?:textarea|input|select)$/i, rperiod = /\./g, rspaces = / /g, rescape = /[^\w\s.|`]/g, fcleanup = function(
				nm) {
			return nm.replace(rescape, "\\$&");
		};

		/*
		 * A number of helper functions used for managing events. Many of the
		 * ideas behind this code originated from Dean Edwards' addEvent
		 * library.
		 */
		jQuery.event = {
			...
			add : function(elem, types, handler, data) {
				...
				if (!eventHandle) {
					elemData.handle = eventHandle = function(e) {
						...
						return typeof jQuery !== "undefined"
								&& (!e || jQuery.event.triggered !== e.type) ? jQuery.event.handle
				...
				while ((type = types[i++])) {
					...
					// Keep track of which events have been used, for event
					// optimization
			...
			remove : function(elem, types, handler, pos) {
				...
				var ret, type, fn, j, i = 0, all, namespaces, namespace, special, eventType, handleObj, origType, elemData = jQuery
						.hasData(elem)
						&& jQuery._data(elem), events = elemData
						&& elemData.events;
				...
				while ((type = types[i++])) {
					...
					if (!all) {
						...
						namespace = new RegExp("(^|\\.)"
								+ jQuery.map(namespaces.slice(0).sort(),
										fcleanup).join("\\.(?:.*\\.)?")
								+ "(\\.|$)");
					}
			...
			// Events that are safe to short-circuit if no handlers are
			// attached.
			// Native DOM events should not be added, they may have inline
			// handlers.
			customEvent : {
				"getData" : true,
				"setData" : true,
			...
			trigger : function(event, data, elem, onlyHandlers) {
				...
				if (type.indexOf(".") >= 0) {
					// Namespaced trigger; create a regexp to match event type
					// in
					// handle()
					namespaces = type.split(".");
					type = namespaces.shift();
				...
				// triggerHandler() and global events don't bubble or run the
				// default action
				...
				if (!elem) {
					// TODO: Stop taunting the data cache; remove global events
					// and
				...
				// Fire event on the current element, then bubble up the DOM
				// tree
				do {
					...
					if (ontype && jQuery.acceptData(cur) && cur[ontype]
					...
					cur = cur.parentNode || cur.ownerDocument
							|| cur === event.target.ownerDocument && window;
				...
				if (!event.isDefaultPrevented()) {
					...
					if ((!special._default || special._default.call(
							elem.ownerDocument, event) === false)
						...
						// name
						// name as the event.
						// Can't use an .isFunction)() check here because IE6/7
						// fails that test.
						// IE<9 dies on focus to hidden element (#1486), may
						// want to
						// revisit a try/catch.
						try {
							if (ontype && elem[type]) {
			...
			handle : function(event) {
				...
				// Snapshot the handlers list since a called handler may
				// add/remove
				// events.
				var handlers = ((jQuery._data(this, "events") || {})[event.type] || [])
						.slice(0), run_all = !event.exclusive
						&& !event.namespace, args = Array.prototype.slice.call(
						arguments, 0);
				...
				for ( var j = 0, l = handlers.length; j < l; j++) {
					...
					// Triggered event must 1) be non-exclusive and have no
					// namespace, or
				...
				live : {
					add : function(handleObj) {
						jQuery.event.add(this, liveConvert(handleObj.origType,
								handleObj.selector), jQuery.extend({},
								handleObj, {
									handler : liveHandler,
					...
					remove : function(handleObj) {
						jQuery.event.remove(this, liveConvert(
								handleObj.origType, handleObj.selector),
								handleObj);
					}
				},
		...
		jQuery.removeEvent = document.removeEventListener ? function(elem,
				type, handle) {
		...
		var withinElement = function(event) {
			...
			try {
				...
				// assuming we've left the element since we most likely
				// mousedover a
				// xul element
			} catch (e) {
			}
		...
		if (!jQuery.support.changeBubbles) {
			...
			jQuery.event.special.change = {
				filters : {
					...
					click : function(e) {
						var elem = e.target, type = jQuery.nodeName(elem,
								"input") ? elem.type : "";
					...
					keydown : function(e) {
						...
						if ((e.keyCode === 13 && !jQuery.nodeName(elem,
								"textarea"))
					...
					// Beforeactivate happens also before the previous element
					// is
					// blurred
		...
		function trigger(type, elem, args) {
			...
			// donor.
			// Don't pass args or remember liveFired; they apply to the donor
			// event.
		...
		if (!jQuery.support.focusinBubbles) {
			jQuery.each({
				...
				function handler(donor) {
					// Donor event is always a native one; fix it and switch its
					// type.
					// Let focusin/out handler cancel the donor focus/blur
					// event.
				...
				.extend({
					...
					toggle : function(fn) {
						...
						var args = arguments, guid = fn.guid || jQuery.guid++, i = 0, toggler = function(
							...
							var lastToggle = (jQuery.data(this, "lastToggle"
									+ fn.guid) || 0)
									% i;
							jQuery.data(this, "lastToggle" + fn.guid,
									lastToggle + 1);
		...
		function liveHandler(event) {
			...
			if (event.liveFired === this || !events || !events.live
					|| event.target.disabled || event.button
					&& event.type === "click") {
				return;
			}
				...
				for (j = 0; j < live.length; j++) {
					...
					if (close.selector === handleObj.selector
						...
						if (handleObj.preType === "mouseenter"
							...
							// Make sure not to accidentally match a child
							// element
				...
				.each(
						("blur focus focusin focusout load resize scroll unload click dblclick "
							...
							jQuery.fn[name] = function(data, fn) {
								...
								return arguments.length > 0 ? this.bind(name,
		...
		/*
		 * ! Sizzle CSS Selector Engine Copyright 2011, The Dojo Foundation
		...
		(function() {
			...
			var Sizzle = function(selector, context, results, seed) {
				...
				if (parts.length > 1 && origPOS.exec(selector)) {
					...
					if (parts.length === 2 && Expr.relative[parts[0]]) {
						...
						set = Expr.relative[parts[0]] ? [ context ] : Sizzle(
								parts.shift(), context);
					...
					// is
					// an ID
					// (but not if it'll be faster if the inner selector is an
					// ID)
					...
					ATTR : function(elem, match) {
						var name = match[1], result = Expr.attrHandle[name] ? Expr.attrHandle[name]
								...
								: elem[name] != null ? elem[name] : elem
										.getAttribute(name), value = result
			...
			if (document.documentElement.compareDocumentPosition) {
				...
				sortOrder = function(a, b) {
					...
					if (a === b) {
						...
						// Fallback to using sourceIndex (in IE) if it's
						// available
						// on both nodes
			...
			(function() {
				// We're going to inject a fake input element with a specified
				// name
				...
				// Inject it into the root element, check its status, and remove
				// it
				// quickly
			...
			Sizzle.isXML = function(elem) {
				// documentElement is verified for cases where it doesn't yet
				// exist
			...
			var posProcess = function(selector, context) {
				...
				// And so must :not(positional) so we move all PSEUDOs to the
				// end
		...
		jQuery.fn
				.extend({
						...
						if (jQuery.isArray(selectors)) {
							...
							if (cur && selectors.length) {
								for (i = 0, l = selectors.length; i < l; i++) {
									...
									if (!matches[selector]) {
										matches[selector] = POS.test(selector) ? jQuery(
												selector, context
														|| this.context)
												: selector;
									}
					...
					add : function(selector, context) {
						var set = typeof selector === "string" ? jQuery(
								selector, context) : jQuery.makeArray(selector
		...
		jQuery
				.extend({
					filter : function(expr, elems, not) {
						...
						return elems.length === 1 ? jQuery.find
								.matchesSelector(elems[0], expr) ? [ elems[0] ]
				...
				.extend({
					...
					before : function() {
						if (this[0] && this[0].parentNode) {
							return this.domManip(arguments, false, function(
									elem) {
					...
					replaceWith : function(value) {
						if (this[0] && this[0].parentNode) {
							// Make sure that the elements are removed from the
							// DOM
					...
					domManip : function(args, table, callback) {
						...
						if (jQuery.isFunction(value)) {
							return this.each(function(i) {
								var self = jQuery(this);
								args[0] = value.call(this, i, table ? self
										.html() : undefined);
		...
		function root(elem, cur) {
			return jQuery.nodeName(elem, "table") ? (elem
					...
					.appendChild(elem.ownerDocument.createElement("tbody")))
					: elem;
		}
		...
		function cloneCopyEvent(src, dest) {
			...
			var internalKey = jQuery.expando, oldData = jQuery.data(src), curData = jQuery
					.data(dest, oldData);

			// Switch to use the internal data object, if it exists, for the
			// next
					...
					for ( var type in events) {
						for ( var i = 0, l = events[type].length; i < l; i++) {
							jQuery.event.add(dest, type
									...
									+ events[type][i].namespace,
									events[type][i], events[type][i].data);
						}
					}
				}
			}
		...
		function cloneFixAttributes(src, dest) {
			...
			if (nodeName === "object") {
				...
				// IE6-8 fails to return the selected option to the default
				// selected
		...
		jQuery.buildFragment = function(args, nodes, scripts) {
			...
			// IE 6 doesn't like it when you put <object> or <embed> elements in
			// a
			// fragment
				...
				.extend({
					clone : function(elem, dataAndEvents, deepDataAndEvents) {
						...
						if ((!jQuery.support.noCloneEvent || !jQuery.support.noCloneChecked)
							...
							// cloneNode.
							// Calling detachEvent on the clone will also remove
							// the
							// events
							// from the original. In order to get around this,
							// we
							// use some
							...
							// Weird iteration because IE will replace the
							// length
							// property
							// with an element if you are cloning the body and
							// one
							// of the
					...
					clean : function(elems, context, fragment, scripts) {
						...
						for ( var i = 0, elem; (elem = elems[i]) != null; i++) {
							...
							// Resets defaultChecked for any radios and
							// checkboxes
						...
						for ( var i = 0, elem; (elem = elems[i]) != null; i++) {
							...
							if (id) {
								...
								if (data && data.events) {
									...
									// Null the DOM reference to avoid IE6/7/8
									// leak
									// (#7054)
									if (data.handle) {
		...
		var ralpha = /alpha\([^)]*\)/i, ropacity = /opacity=([^)]*)/, rdashAlpha = /-([a-z])/ig,
		// fixed for IE9, see #8346
		rupper = /([A-Z]|^ms)/g, rnumpx = /^-?\d+(?:px)?$/i, rnum = /^-?\d/, rrelNum = /^[+\-]=/, rrelNumFilter = /[^+\-\.\de]+/g,
				...
				.extend({
					...
					style : function(elem, name, value, extra) {
						...
						if (value !== undefined) {
							...
							// Make sure that NaN and null values aren't set.
							// See:
							// #7116
							...
							// If a number was passed in, add 'px' to the
							// (except
							...
							// Otherwise just get the value from the style
							// object
							return style[name];
						}
					},
					...
					css : function(elem, name, extra) {
						...
						// If a hook was provided get the computed value from
						// there
						if (hooks
								&& "get" in hooks
							...
							// Otherwise, if a way to get the computed value
							// exists,
							// use that
						} else if (curCSS) {
		...
		jQuery(function() {
			// This hook cannot be added until DOM ready because the support
			// test
		...
		if (document.documentElement.currentStyle) {
			currentStyle = function(elem, name) {
				var left, ret = elem.currentStyle && elem.currentStyle[name], rsLeft = elem.runtimeStyle
						&& elem.runtimeStyle[name], style = elem.style;
		...
		function getWH(elem, name, extra) {
			...
			jQuery.each(which, function() {
				...
				if (extra === "margin") {
					...
					val -= parseFloat(jQuery.css(elem, "border" + this
							+ "Width")) || 0;
				}
			});
		...
		rlocalProtocol = /^(?:about|app|app\-storage|.+\-extension|file|widget):$/, rnoContent = /^(?:GET|HEAD)$/, rprotocol = /^\/\//, rquery = /\?/, rscript = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, rselectTextarea = /^(?:select|textarea)/i, rspacesAjax = /\s+/, rts = /([?&])_=[^&]*/, rurl = /^([\w\+\.\-]+:)(?:\/\/([^\/?#:]*)(?::(\d+))?)?/,
		 ...
		 * ajax/jsonp.js for an example) 2) These are called: - BEFORE asking
		 * for a transport - AFTER param serialization (s.data is a string if
		 * s.processData is true) 3) key is the dataType 4) the catchall symbol
		 * "*" can be used 5) execution will start with transport dataType and
		 * THEN continue down to "*" if needed
		 */
		prefilters = {},

		/*
		 * Transports bindings 1) key is the dataType 2) the catchall symbol "*"
		 * can be used 3) selection will start with transport dataType and THEN
		 * go to "*" if needed
		 */
		transports = {},
		...
		function inspectPrefiltersOrTransports(structure, options,
				originalOptions, jqXHR, dataType /* internal */, inspected /* internal */) {
			...
			var list = structure[dataType], i = 0, length = list ? list.length
				...
				.extend({
					load : function(url, params, callback) {
						if (typeof url !== "string" && _load) {
							...
							// Don't do a request if no elements are being
							// requested
						} else if (!this.length) {
							return this;
						}
						...
						jQuery.ajax({
							...
							dataType : "html",
							data : params,
							// Complete callback (responseText is used
							// internally)
				...
				.extend({
					...
					ajaxSetup : function(target, settings) {
						if (!settings) {
							...
							settings = target;
							target = jQuery.extend(true, jQuery.ajaxSettings,
									settings);
						} else {
							// target was provided, we extend into it
							jQuery.extend(true, target, jQuery.ajaxSettings,
									settings);
						}
					...
					ajaxSettings : {
						...
						async : true,
						/*
						 * timeout: 0, data: null, dataType: null, username:
						 * null, password: null, cache: null, traditional:
						 * false, headers: {},
						 */
						...
						// 1) key format is "source_type destination_type" (a
						// single
						// space in-between)
						// 2) the catchall symbol "*" can be used for
						// source_type
						converters : {
					...
					ajax : function(url, options) {
						...
						function done(status, statusText, responses, headers) {
							...
							// Dereference transport for early garbage
							// collection
							// (no matter how long the jqXHR object will be
							// used)
							transport = undefined;
							...
							var isSuccess, success, error, response = responses ? ajaxHandleResponses(
									s, jqXHR, responses)
						...
						if (!s.hasContent) {
							...
							if (s.cache === false) {
								...
								// if nothing was replaced, add timestamp to the
								// end
								s.url = ret
										+ ((ret === s.url) ? (rquery
												.test(s.url) ? "&" : "?")
						...
						if (s.beforeSend
								&& (s.beforeSend
										.call(callbackContext, jqXHR, s) === false || state === 2)) {
		 ...
		 * accordingly - finds the right dataType (mediates between content-type
				...
				.ajaxPrefilter(
						...
						function(s, originalSettings, jqXHR) {
							...
							if (s.dataTypes[0] === "jsonp"
								...
								// Use data converter to retrieve json after
								// script
								// execution
		 ...
		 * (can't request local files), so we use the ActiveXObject when it is
		 * available Additionally XMLHttpRequest can be disabled in IE7/IE8 so
		 * we need a fallback.
		 */
		function() {
						...
						if (!s.crossDomain || jQuery.support.cors) {
							...
							return {
								send : function(headers, complete) {
									...
									// akin to a jigsaw puzzle, we simply never
									// set
									// it to be sure.
									...
									// Do send the request
									// This may raise an exception which is
									// actually
									// handled in jQuery.ajax (so no try/catch
									// here)
									...
									callback = function(_, isAbort) {
										...
										// Firefox throws exceptions when
										// accessing
										// properties
										// of an xhr when a network error
										// occured
										// http://helpful.knobs-dials.com/index.php/Component_returned_failure_code:_0x80040111_(NS_ERROR_NOT_AVAILABLE)
										try {

											// Was never called and is aborted
											// or
											// complete
											if (callback
									...
									// and has been retrieved directly (IE6 &
									// IE7)
									...
									if (!s.async || xhr.readyState === 4) {
										...
										handle = ++xhrId;
										if (xhrOnUnloadAbort) {
											// Create the active xhrs callbacks
											// list
											// if needed
		...
		var elemdisplay = {}, iframe, iframeDoc, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = /^([+\-]=)?([\d+.\-]+)([a-z%]*)$/i, timerId, fxAttrs = [
				...
				[ "width", "marginLeft", "marginRight", "paddingLeft",
						"paddingRight" ],
				// opacity animations
				[ "opacity" ] ], fxNow, requestAnimationFrame = window.webkitRequestAnimationFrame
				...
				.extend({
						...
						if (speed || speed === 0) {
							...
							for ( var i = 0, j = this.length; i < j; i++) {
								...
								if (elem.style) {
									...
									// Reset the inline display of this element
									// to
									// learn if it is
									...
									// Set elements which have been overridden
									// with
									// display: none
							...
							// Set the display of most of the elements in a
							// second
							// loop
					...
					animate : function(prop, speed, easing, callback) {
						...
						return this[optall.queue === false ? "each" : "queue"]
								(function() {
									// XXX 'this' does not always have a
									// nodeName
									// when running the
									// test suite
									...
									var opt = jQuery.extend({}, optall), isElement = this.nodeType === 1, hidden = isElement
											&& jQuery(this).is(":hidden"), name, val, p, display, e, parts, start, end, unit;

									// will store per property easing and be
									// used to
									...
									for (p in prop) {
										...
										if (isElement
											...
											// because IE does not
											// change the overflow attribute
											// when
											// overflowX and
											// overflowY are set to the same
											// value
											opt.overflow = [
													this.style.overflow,
		...
		jQuery.fx.prototype = {
			...
			cur : function() {
				...
				return isNaN(parsed = parseFloat(r)) ? !r || r === "auto" ? 0
						: r : parsed;
			},
			...
			custom : function(from, to, unit) {
				...
				if (t() && jQuery.timers.push(t) && !timerId) {
					// Use requestAnimationFrame instead of setInterval if
					// available
					if (requestAnimationFrame) {
						timerId = 1;
						raf = function() {
							// When timerId gets set to null at any point, this
							// stops
							if (timerId) {
		...
		function defaultDisplay(nodeName) {
			...
			if (!elemdisplay[nodeName]) {
				...
				if (display === "none" || display === "") {
					...
					// Create a cacheable copy of the iframe document on first
					// call.
		...
		if ("getBoundingClientRect" in document.documentElement) {
			jQuery.fn.offset = function(options) {
				...
				var body = doc.body, win = getWindow(doc), clientTop = docElem.clientTop
						...
						|| body.scrollLeft, top = box.top + scrollTop
						- clientTop, left = box.left + scrollLeft - clientLeft;
		...
		jQuery.offset = {
			initialize : function() {
				var body = document.body, container = document
						.createElement("div"), innerDiv, checkDiv, table, td, bodyMarginTop = parseFloat(jQuery
			...
			setOffset : function(elem, options, i) {
				...
				var curElem = jQuery(elem), curOffset = curElem.offset(), curCSSTop = jQuery
						.css(elem, "top"), curCSSLeft = jQuery
		...
		jQuery.fn
				.extend({
					position : function() {
						...
						offset.left -= parseFloat(jQuery
								.css(elem, "marginLeft")) || 0;

						// Add offsetParent borders
						parentOffset.top += parseFloat(jQuery.css(
								offsetParent[0], "borderTopWidth")) || 0;
						parentOffset.left += parseFloat(jQuery.css(
								offsetParent[0], "borderLeftWidth")) || 0;
					...
					offsetParent : function() {
						return this
								.map(function() {
									...
									while (offsetParent
											&& (!rroot
													.test(offsetParent.nodeName) && jQuery
													.css(offsetParent,
															"position") === "static")) {
		...
		jQuery.each([ "Height", "Width" ], function(i, name) {
			...
			jQuery.fn["inner" + name] = function() {
				return this[0] ? parseFloat(jQuery
						.css(this[0], type, "padding")) : null;
			};
			...
			jQuery.fn[type] = function(size) {
				...
				if (jQuery.isWindow(elem)) {
					...
					return this.css(type, typeof size === "string" ? size
							: size + "px");
				}
			};
		...
		window.bq_jQuery = window.bq_$ = jQuery;
