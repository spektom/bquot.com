/*
 * AJAXSLT - Pure javascript implementation of XSLT and XPath 
 *
...
(function(window) {
	...
	/**
	 * This is an optimization for copying attribute lists in IE. IE includes
	 * many extraneous properties in its DOM attribute lists, which take require
	...
	function copyArrayIgnoringAttributesWithoutValue(dst, src) {
		if (!src)
			return;
		for ( var i = src.length - 1; i >= 0; --i) {
			// this test will pass so long as the attribute has a non-empty
			// string
	...
	// default; they can be disabled by passing "true" in as the second
	// parameter.
	 ...
	 * other nodes: for the document node, the owner document is the node
	 * itself, for all others it's the ownerDocument property.
	 * 
	 * @param {Node}
	 *            node
	 * @return {Document}
	...
	/**
	 * Escape the special regular expression characters when the regular
	 * expression is specified as a string.
	 * 
	...
	/**
	 * Determines whether a predicate expression contains a "positional
	 * selector". A positional selector filters nodes from the nodelist input
	 * based on their position within that list. When such selectors are
	 * encountered, the evaluation of the predicate cannot be depth-first,
	 * because the positional selector may be based on the result of evaluating
	 * predicates that precede it.
	 */
	function predicateExprHasPositionalSelector(expr, isRecursiveCall) {
		...
		if (expr instanceof BinaryExpr) {
			return (predicateExprHasPositionalSelector(expr.expr1, true) || predicateExprHasPositionalSelector(
					expr.expr2, true));
		}
		return false;
	}
	...
	var XML10_VERSION_INFO = XML_S + 'version' + XML_EQ + '("1\\.0"|'
			+ "'1\\.0')";
	var XML10_BASE_CHAR = (REGEXP_UNICODE) ? '\u0041-\u005a\u0061-\u007a\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u00ff'
			+ '\u0100-\u0131\u0134-\u013e\u0141-\u0148\u014a-\u017e\u0180-\u01c3'
			+ '\u01cd-\u01f0\u01f4-\u01f5\u01fa-\u0217\u0250-\u02a8\u02bb-\u02c1\u0386'
			+ '\u0388-\u038a\u038c\u038e-\u03a1\u03a3-\u03ce\u03d0-\u03d6\u03da\u03dc'
			+ '\u03de\u03e0\u03e2-\u03f3\u0401-\u040c\u040e-\u044f\u0451-\u045c'
	...
	var XML10_IDEOGRAPHIC = (REGEXP_UNICODE) ? '\u4e00-\u9fa5\u3007\u3021-\u3029'
			: '';
	var XML10_COMBINING_CHAR = (REGEXP_UNICODE) ? '\u0300-\u0345\u0360-\u0361\u0483-\u0486\u0591-\u05a1\u05a3-\u05b9'
			+ '\u05bb-\u05bd\u05bf\u05c1-\u05c2\u05c4\u064b-\u0652\u0670\u06d6-\u06dc'
			+ '\u06dd-\u06df\u06e0-\u06e4\u06e7-\u06e8\u06ea-\u06ed\u0901-\u0903\u093c'
			+ '\u093e-\u094c\u094d\u0951-\u0954\u0962-\u0963\u0981-\u0983\u09bc\u09be'
			+ '\u09bf\u09c0-\u09c4\u09c7-\u09c8\u09cb-\u09cd\u09d7\u09e2-\u09e3\u0a02'
			...
			+ '\u0fb1-\u0fb7\u0fb9\u20d0-\u20dc\u20e1\u302a-\u302f\u3099\u309a'
			: '';
	var XML10_DIGIT = (REGEXP_UNICODE) ? '\u0030-\u0039\u0660-\u0669\u06f0-\u06f9\u0966-\u096f\u09e6-\u09ef'
			+ '\u0a66-\u0a6f\u0ae6-\u0aef\u0b66-\u0b6f\u0be7-\u0bef\u0c66-\u0c6f'
			+ '\u0ce6-\u0cef\u0d66-\u0d6f\u0e50-\u0e59\u0ed0-\u0ed9\u0f20-\u0f29'
			: '0-9';
	var XML10_EXTENDER = (REGEXP_UNICODE) ? '\u00b7\u02d0\u02d1\u0387\u0640\u0e46\u0ec6\u3005\u3031-\u3035'
			+ '\u309d-\u309e\u30fc-\u30fe'
			: '';
	...
	var XML10_ATT_VALUE = '"(([^<&"]|' + XML10_REFERENCE + ')*)"|'
			+ "'(([^<&']|" + XML10_REFERENCE + ")*)'";
	var XML10_ATTRIBUTE = '(' + XML10_NAME + ')' + XML_EQ + '('
			+ XML10_ATT_VALUE + ')';
...
// surrogate pairs, but any browser doesn't support surrogate paris in
	...
	var XML11_VERSION_INFO = XML_S + 'version' + XML_EQ + '("1\\.1"|'
			+ "'1\\.1')";
	var XML11_NAME_START_CHAR = (REGEXP_UNICODE) ? ':A-Z_a-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u02ff\u0370-\u037d'
			+ '\u037f-\u1fff\u200c-\u200d\u2070-\u218f\u2c00-\u2fef\u3001-\ud7ff'
			+ '\uf900-\ufdcf\ufdf0-\ufffd'
			: ':A-Z_a-z';
	var XML11_NAME_CHAR = XML11_NAME_START_CHAR
			+ ((REGEXP_UNICODE) ? '\\.0-9\u00b7\u0300-\u036f\u203f-\u2040-'
					: '\\.0-9-');
	...
	var XML11_ATT_VALUE = '"(([^<&"]|' + XML11_REFERENCE + ')*)"|'
			+ "'(([^<&']|" + XML11_REFERENCE + ")*)'";
	var XML11_ATTRIBUTE = '(' + XML11_NAME + ')' + XML_EQ + '('
			+ XML11_ATT_VALUE + ')';
		...
		for ( var i = 1; i < parts.length; ++i) {
			...
			switch (entityName) {
				...
				default:
					...
					// the entity text through non-W3C DOM properties and read
					// it
	...
	XNode.prototype.setAttribute = function(name, value) {
		...
		this.attributes.push(XNode
				.create(DOM_ATTRIBUTE_NODE, name, value, this));
	}
	...
	XDocument.prototype.createCDATASection = function(data) {
		return XNode.create(DOM_CDATA_SECTION_NODE, '#cdata-section', data,
				this);
	}
	...
	function xpathParse(expr) {
		...
		while (!done) {
			...
			if (rule
					...
					&& (!previous || previous.tag == TOK_AT
							|| previous.tag == TOK_DSLASH
							|| previous.tag == TOK_SLASH
							|| previous.tag == TOK_AXIS || previous.tag == TOK_DOLLAR)) {
				rule = TOK_QNAME;
			}
			...
			if (rule) {
				...
				ahead = {
					tag : rule,
					match : match,
					prec : rule.prec ? rule.prec : 0, // || 0 is removed by
					// the compiler
	...
	/*
	 * DGF xpathReduce is where the magic happens in this parser. Skim down to
	 * the bottom of this file to find the table of grammatical rules and
	 * precedence numbers, "The productions of the grammar".
	 * 
	 * The idea here is that we want to take a stack of tokens and apply
	 * grammatical rules to them, "reducing" them to higher-level tokens.
	 * Ultimately, any valid XPath should reduce to exactly one "Expr" token.
	 * 
	 * Reduce too early or too late and you'll have two tokens that can't reduce
	 * to single Expr. For example, you may hastily reduce a qname that should
	 * name a function, incorrectly treating it as a tag name. Or you may reduce
	 * too late, accidentally reducing the last part of the XPath into a
	 * top-level "Expr" that won't reduce with earlier parts of the XPath.
	 * 
	 ...
	 * token has a higher precedence number than the rule candidate, we'll
	 * "shift" the token onto the token stack, instead of immediately applying
	 * the rule candidate.
	 * 
	...
	function xpathReduce(stack, ahead) {
		...
		var ret;
		if (cand
				&& (!ahead || cand.prec > ahead.prec || (ahead.tag.left && cand.prec >= ahead.prec))) {
			...
			xpathLog('reduce '
					...
					+ cand.prec
					+ ' ahead '
					+ (ahead ? ahead.tag.label + ' ' + ahead.prec
							+ (ahead.tag.left ? ' left' : '') : ' none '));

			var matchexpr = mapExpr(cand.match, function(m) {
				return m.expr;
			});
	...
	function xpathMatchStack(stack, pattern) {
		...
		for (p = P - 1, s = S - 1; p >= 0 && s >= 0; --p, s -= ds) {
			...
			qmatch.expr = mapExpr(qmatch, function(m) {
				return m.expr;
			});
		}
	...
	// as a result is found. This is an optimization that might make sense if
	// you
	...
	function ExprContext(node, opt_position, opt_nodelist, opt_parent,
		...
		if (opt_parent) {
			...
			if (node.nodeType == DOM_DOCUMENT_NODE) {
				...
				// document is null. Our root, however is the document that we
				// are
				// processing, so the initial context is created from its
				// document
	...
	ExprContext.prototype.clone = function(opt_node, opt_position, opt_nodelist) {
		return new ExprContext(opt_node || this.node,
				typeof opt_position != 'undefined' ? opt_position
						: this.position, opt_nodelist || this.nodelist, this,
				this.caseInsensitive, this.ignoreAttributesWithoutValue,
				this.returnOnFirstMatch, this.ignoreNonElementNodesForNTA);
	};
	...
	ExprContext.prototype.setIgnoreNonElementNodesForNTA = function(
	...
	LocationExpr.prototype.appendStep = function(s) {
		var combinedStep = this._combineSteps(
				this.steps[this.steps.length - 1], s);
		if (combinedStep) {
		...
		if (prevStep.nodetest instanceof NodeTestAny && !hasPredicates) {
			...
			if (prevStep.axis == xpathAxis.DESCENDANT_OR_SELF) {
				if (nextStep.axis == xpathAxis.CHILD) {
					// HBC - commenting out, because this is not a valid
					// reduction
	...
	function xPathStep(nodes, steps, step, input, ctx) {
		...
		if (ctx.returnOnFirstMatch && !s.hasPositionalPredicate) {
			...
			// the predicates were not processed in the last evaluate(), so that
			// we can
			// process them here with the returnOnFirstMatch optimization. We do
			// a
			// depth-first grab at any nodes that pass the predicate tests.
			// There is no
			// way to optimize when predicates contain positional selectors,
			// including
			// indexes or uses of the last() or position() functions, because
			// they
			// typically require the entire nodelist for context. Process
			// without
			...
			nodelistLoop: for ( var i = 0; i < nLength; ++i) {
				...
				for ( var j = 0; j < pLength; ++j) {
					if (!s.predicate[j].evaluate(ctx.clone(n, i, nodelist))
							.booleanValue()) {
			...
			// set returnOnFirstMatch to false for the cloned ExprContext,
			// because
			// behavior in StepExpr.prototype.evaluate is driven off its value.
			// Note
	...
	StepExpr.prototype.evaluate = function(ctx) {
		...
		if (this.axis == xpathAxis.ANCESTOR_OR_SELF) {
			...
			if (this.nodetest.name != undefined) {
					...
					if (input.attributes instanceof Array) {
						...
						if (this.nodetest.name == 'style') {
							...
							if (value && typeof (value) != 'string') {
								// this is the case where indexing into the
								// attributes array
								// doesn't give us the attribute node in IE - we
								// create our own
								// node instead
								nodelist.push(XNode.create(DOM_ATTRIBUTE_NODE,
										'style', value.cssText, document));
							} else {
								nodelist
										.push(input.attributes[this.nodetest.name]);
							}
						} else {
				...
				if (ctx.ignoreAttributesWithoutValue) {
					copyArrayIgnoringAttributesWithoutValue(nodelist,
							input.attributes);
				} else {
					copyArray(nodelist, input.attributes);
				}
			...
			var tagName = xpathExtractTagNameFromNodeTest(this.nodetest,
					ctx.ignoreNonElementNodesForNTA);
		...
		if (!skipNodeTest) {
			...
			for ( var i = 0; i < nodelist0.length; ++i) {
				var n = nodelist0[i];
				if (this.nodetest.evaluate(ctx.clone(n, i, nodelist0))
						.booleanValue()) {
					nodelist.push(n);
				}
			}
		...
		if (!ctx.returnOnFirstMatch) {
			for ( var i = 0; i < this.predicate.length; ++i) {
				...
				for ( var ii = 0; ii < nodelist0.length; ++ii) {
					var n = nodelist0[ii];
					if (this.predicate[i].evaluate(ctx.clone(n, ii, nodelist0))
							.booleanValue()) {
						nodelist.push(n);
					}
				}
	...
	NodeTestPI.prototype.evaluate = function(ctx) {
		return new BooleanValue(
				ctx.node.nodeType == DOM_PROCESSING_INSTRUCTION_NODE
	...
	PathExpr.prototype.evaluate = function(ctx) {
		...
		if (ctx.returnOnFirstMatch) {
			for ( var i = 0; i < nodes.length; ++i) {
				nodes1 = this.rel.evaluate(ctx.clone(nodes[i], i, nodes))
						.nodeSetValue();
				if (nodes1.length > 0) {
					break;
				}
			...
			for ( var i = 0; i < nodes.length; ++i) {
				var nodes0 = this.rel.evaluate(ctx.clone(nodes[i], i, nodes))
						.nodeSetValue();
	...
	FilterExpr.prototype.evaluate = function(ctx) {
		...
		for ( var i = 0; i < this.predicate.length; ++i) {
			...
			for ( var j = 0; j < nodes0.length; ++j) {
				var n = nodes0[j];
				if (this.predicate[i].evaluate(ctx.clone(n, j, nodes0))
						.booleanValue()) {
					nodes.push(n);
				}
			}
			...
			for ( var i1 = 0; i1 < n1.length; ++i1) {
				for ( var i2 = 0; i2 < n2.length; ++i2) {
					if (cmp(xmlValue(n1[i1]), xmlValue(n2[i2]))) {
						ret = true;
						// Break outer loop. Labels confuse the jscompiler and
						// we
						// don't use them.
						i2 = n2.length;
						i1 = n1.length;
	...
	var xpathAxesRe = [ xpathAxis.ANCESTOR_OR_SELF, xpathAxis.ANCESTOR,
			xpathAxis.ATTRIBUTE, xpathAxis.CHILD, xpathAxis.DESCENDANT_OR_SELF,
			xpathAxis.DESCENDANT, xpathAxis.FOLLOWING_SIBLING,
			xpathAxis.FOLLOWING, xpathAxis.NAMESPACE, xpathAxis.PARENT,
			xpathAxis.PRECEDING_SIBLING, xpathAxis.PRECEDING, xpathAxis.SELF ]
			.join('|');
	...
	var xpathTokenRules = [ TOK_DSLASH, TOK_SLASH, TOK_DDOT, TOK_DOT, TOK_AXIS,
			TOK_COLON, TOK_AXISNAME, TOK_NODEO, TOK_PARENO, TOK_PARENC,
			TOK_BRACKO, TOK_BRACKC, TOK_AT, TOK_COMMA, TOK_OR, TOK_AND,
			TOK_NEQ, TOK_EQ, TOK_GE, TOK_GT, TOK_LE, TOK_LT, TOK_PLUS,
			TOK_MINUS, TOK_ASTERISK, TOK_PIPE, TOK_MOD, TOK_DIV, TOK_LITERALQ,
			TOK_LITERALQQ, TOK_NUMBER, TOK_QNAME, TOK_NCNAME, TOK_DOLLAR ];
	...
	var XPathRelativeLocationPath = {
		label : "RelativeLocationPath"
	};
	var XPathAbsoluteLocationPath = {
	...
	var XPathFunctionCall = {
		label : "FunctionCall"
	};
	var XPathArgumentRemainder = {
	...
	var xpathNonTerminals = [ XPathLocationPath, XPathRelativeLocationPath,
			XPathAbsoluteLocationPath, XPathStep, XPathNodeTest,
			XPathPredicate, XPathLiteral, XPathExpr, XPathPrimaryExpr,
			XPathVariableReference, XPathNumber, XPathFunctionCall,
			XPathArgumentRemainder, XPathPathExpr, XPathUnionExpr,
	...
	var xpathGrammarRules = [
			...
			[ XPathAbsoluteLocationPath,
					[ TOK_SLASH, XPathRelativeLocationPath ], 18,
					makeLocationExpr1 ],
			[ XPathAbsoluteLocationPath,
					[ TOK_DSLASH, XPathRelativeLocationPath ], 18,
					makeLocationExpr2 ],
			...
			[
					XPathFunctionCall,
					[ TOK_QNAME, TOK_PARENO, XPathExpr, XPathArgumentRemainder,
							Q_MM, TOK_PARENC ], -1, makeFunctionCallExpr2 ],
			...
			[ XPathPathExpr,
					[ XPathFilterExpr, TOK_SLASH, XPathRelativeLocationPath ],
					19, makePathExpr1 ],
			[ XPathPathExpr,
					[ XPathFilterExpr, TOK_DSLASH, XPathRelativeLocationPath ],
					19, makePathExpr2 ],
			...
			[ XPathExpr, [ XPathExpr, TOK_ASTERISK, XPathExpr ], -1,
					makeBinaryExpr, ASSOC_LEFT ],
			[ XPathExpr, [ XPathExpr, TOK_DIV, XPathExpr ], -1, makeBinaryExpr,
					ASSOC_LEFT ],
			[ XPathExpr, [ XPathExpr, TOK_MOD, XPathExpr ], -1, makeBinaryExpr,
					ASSOC_LEFT ],
	 ...
	 * @param nodetest
	 *            the node test
	 * @param ignoreNonElementNodesForNTA
	 *            if true, the node list returned when evaluating "node()" will
	 *            not contain non-element nodes. This can boost performance.
	...
	function xpathResolve(contextNode, xpath, firstMatch) {
	...
	window["bq_xpathResolve"] = xpathResolve;
})(window);
